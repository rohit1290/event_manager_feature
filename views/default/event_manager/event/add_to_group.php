...
if ($add_options || $remove_options) {
	...
	if ($add_options) {
		echo "<h4>".elgg_echo('event_manager:add_to_group:add_group_title')."</h4>";
		$add_pulldown = elgg_view("input/select", array("name" => "group_id", "options_values" => $add_options));
		$submit_button = "<br><br><p>".elgg_view("input/submit", array("value" => elgg_echo('event_manager:add_to_group:add_group_button')))."</p><br>";
		echo elgg_view ('input/form', array("body" => $event_bit.$add_pulldown.$submit_button, "action" => elgg_get_site_url()."action/event_manager/add_to_group"));
	}
	echo '</div>';
}
